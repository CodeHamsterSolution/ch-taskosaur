...
services:
  ...
  app:
    ...
    restart: unless-stopped
    healthcheck:
      test: ["CMD-SHELL", "node -e \"require('http').get('http://localhost:7777/api/health', (r) => process.exit(r.statusCode === 200 ? 0 : 1))\""]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 60s
...
| `FRONTEND_URL` | `http://localhost:7777` | Frontend URL for links |
| `CORS_ORIGIN` | `http://localhost:7777` | CORS allowed origin |
| `NEXT_PUBLIC_API_BASE_URL` | `http://localhost:7777/api` | API endpoint URL |
...
- `7777` - Combined frontend and backend (Next.js serves both)
...
```yaml
ports:
  - "8080:7777"  # Use port 8080 instead
```
